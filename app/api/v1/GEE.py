...
Пожалуйста, перейдите по ссылке чтобы подтвердить ваш email адрес:
{verification_url}
...
async def send_password_reset_email(to: str, reset_token: str):
    RESET_PASSWORD_BASE_URL = 'baikal.ict.nsc.ru/auth/password-recovery'
    reset_password_url = f"{RESET_PASSWORD_BASE_URL}?token={reset_token}"
...
"""
    ...
    <html>
      <body>
        ...
        <a href="{reset_password_url}">Подтвердить Email</a>
        <p>Или скопируйте ссылку: {reset_password_url}</p>
      </body>
    </html>
    """
...
def get_lst_by_period(
    ...
    try:
        ...
        image = dataset.mean()
        if image is None:
            raise HTTPException(status_code=404, detail="Нет данных за указанный период в выбранной точке.")
