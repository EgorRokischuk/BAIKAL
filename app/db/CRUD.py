...
from db.models import External_services, PasswordResetToken
...
async def get_user_by_login_or_email(login_or_email: str, db: AsyncSession = Depends(get_db)):
    # Создаем запрос с помощью select
    stmt = select(User).where(
        or_(
            User.login == login_or_email,
            User.mail == login_or_email
        )
    )
...
async def mark_verification_request_as_used(email: str, db: AsyncSession):
    """Помечает все верификационные запросы для email как использованные"""
...
async def create_password_reset_token(email: str, db: AsyncSession) -> str:
    # Деактивируем старые токены для этого email
...
async def get_valid_reset_token(token: str, db: AsyncSession):
...
async def mark_reset_token_as_used(token: str, db: AsyncSession):
    stmt = update(PasswordResetToken).where(
        PasswordResetToken.token == token
    ).values(is_used=True)
    await db.execute(stmt)
...
async def update_user_password(email: str, new_password: str, db: AsyncSession):
    ...
    stmt = update(User).where(
        User.mail == email
    ).values({column("pass"): hashed_password})
    await db.execute(stmt)
    await db.commit()
